...
void readHDF5(const char *file_name,
              ...
              char *mesh_file, int* source_type, int *voxelization_type,
              // arrays:
              float*& material_coefficients, float*& receiver_vector,
              float*& source_vector, float*& src_data_vector, float*& vertices,
  ...
  H5LTread_dataset_int(file_id, "/voxelization_type", voxelization_type);
  ...
  H5LTread_dataset_float(file_id, "/vertices", vertices);

  // Read the unsigned ones (no function for these dudes):
  hid_t dataset_id = H5Dopen2(file_id, "/triangles", H5P_DEFAULT);
  H5Dread(dataset_id, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, triangles);
  // Read mesh file:
...
void writeResponseHDF5(const char* name, int fs, double dX,
                       ...
                       const char* mesh_file, int CUDA_steps,
...
int main(int argc, char **argv) {
  ...
  readHDF5(filename,
           ...
           mesh_file, &src_type, &voxelization_type,
  ...
  MaterialHandler mh;
  mh.addMaterials(mat_coefs, num_triangles, 20);
  ...
  (L" mesh_file = %s\n dX = %f\n fs = %d\n num_vertices = %i\n"
    ...
    %mesh_file  %dX %fs %num_vertices %num_triangles %c_sound %lambda_sim
  ...
  log_msg<LOG_DEBUG>(
    L"FDTD_MPI - Data loading ok. Voxelizing %s...")
    %(voxelization_type == 0 ? "'SOLID'" : (voxelization_type == 1 ? "'6SEP'" : "'CONS'") );
  ...
  uint3 voxelization_dim_ = get_Geometry_surface_Voxelization_dims(
                                   vertices, triangles, num_triangles,
                                   num_vertices,dX);
  ...
  unsigned char* d_postition_idx_ = (unsigned char*)NULL;
  unsigned char* d_materials_idx_ = (unsigned char*)NULL;

  unsigned char voxelization_result;
  switch(voxelization_type) {
    case 0:
      log_msg<LOG_DEBUG>
        (L"FDTD_MPI - SOLID voxelization is not supported ");
        break;
    case 1:
      voxelization_result = voxelizeGeometry_surface_6_separating(
                             vertices,
                             triangles,
                             material_indices_ptr,
                             num_triangles,
                             num_vertices,
        ...
        log_msg<LOG_DEBUG>(L"FDTD_MPI - 6-SEPARATING voxelization done... "
            "Dimensions of [%d,%d,%d]") %(int)voxelization_dim_.x
        %(int)voxelization_dim_.y %(int)voxelization_dim_.z;
        break;
    case 2:
      voxelization_result = voxelizeGeometry_surface_conservative(
                           vertices,
                           triangles,
                           material_indices_ptr,
                           num_triangles,
      ...
      log_msg<LOG_DEBUG>(L"FDTD_MPI - CONSERVATIVE voxelization done... "
          "Dimensions of [%d,%d,%d]") %(int)voxelization_dim_.x
            %(int)voxelization_dim_.y %(int)voxelization_dim_.z;
      break;
  }

  unsigned int mem_size = (int)voxelization_dim_.x *
                          (int)voxelization_dim_.y * (int)voxelization_dim_.z;

  unsigned int dimXY = (int)voxelization_dim_.x * (int)voxelization_dim_.y;
  ...
  if ( device_count == 1 && GPU_partitions == 2) {
    ...
    the_mesh->makePartition(GPU_partitions, d_postition_idx_,
                            d_materials_idx_, v_);
  }
  else {
      the_mesh->makePartition(GPU_partitions, d_postition_idx_,
